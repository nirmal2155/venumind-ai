...
const SECTORS_DATA = [
  ...
  { id: 'East', name: 'East Stand (Blocks 140-155)', density: '62%', status: '✅ Nominal', color: 'var(--accent-green)' },
  { id: 'West', name: 'West Stand (VIP & Press)', density: '45%', status: '✅ Nominal', color: 'var(--accent-green)' }
];
...
const DigitalTwinView = () => {
  ...
  return (
    <div style={{
      ...
      <h3 style={{ margin: '0 0 12px 0', fontSize: '0.8rem', letterSpacing: '2px', color: 'var(--accent-blue)', display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        <div style={{
          ...
          <svg width="150" height="150" viewBox="0 0 100 100" data-testid="stadium-svg">
            ...
            <path d="M85 35 A 40 40 0 0 1 85 65 L 50 50 Z" 
                  fill={hoveredSector === 'East' ? 'rgba(5, 150, 105,0.3)' : 'rgba(5, 150, 105,0.15)'} 
                  stroke="var(--accent-green)" strokeWidth="1" style={{ cursor: 'pointer', transition: 'all 0.3s' }}
                  onMouseEnter={() => setHoveredSector('East')}
                  onMouseLeave={() => setHoveredSector(null)}
            />
            {/* West Stand */}
            <path d="M15 35 A 40 40 0 0 0 15 65 L 50 50 Z" 
                  fill={hoveredSector === 'West' ? 'rgba(5, 150, 105,0.3)' : 'rgba(5, 150, 105,0.15)'} 
                  stroke="var(--accent-green)" strokeWidth="1" style={{ cursor: 'pointer', transition: 'all 0.3s' }}
          ...
          <div style={{ position: 'absolute', bottom: '6px', fontSize: '0.65rem', color: 'var(--text-muted)', fontFamily: 'monospace' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', justifyContent: 'center' }}>
          {hoveredSector ? (
            (() => {
              ...
              return (
                <div style={{ background: 'rgba(30, 64, 175, 0.03)', border: `1px solid ${sec.color}`, padding: '10px', borderRadius: '8px', animation: 'fadeIn 0.2s' }}>
                  <div style={{ fontSize: '0.75rem', fontWeight: 'bold', color: 'var(--text-primary)' }}>{sec.name}</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'rgba(30, 64, 175, 0.03)', padding: '6px 10px', borderRadius: '8px' }}>
                <Cloud size={14} color="var(--accent-blue)" />
                <div style={{ fontSize: '0.7rem' }}>
                  <span style={{ color: 'var(--text-secondary)', display: 'block' }}>BOWL WEATHER</span>
                  <strong style={{ color: 'var(--text-primary)' }}>22.4°C | 54% Hum</strong>
                </div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'rgba(30, 64, 175, 0.03)', padding: '6px 10px', borderRadius: '8px' }}>
                <Car size={14} color="var(--accent-green)" />
                <div style={{ fontSize: '0.7rem' }}>
                  <span style={{ color: 'var(--text-secondary)', display: 'block' }}>PARKING LOT A & B</span>
                  <strong style={{ color: 'var(--text-primary)' }}>84% & 91% Slots occupied</strong>
                </div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'rgba(30, 64, 175, 0.03)', padding: '6px 10px', borderRadius: '8px' }}>
